...
@Component({
  selector: 'app-list-injury-physo',
  templateUrl: './list-injury-physo.component.html',
  styleUrls: ['./list-injury-physo.component.scss']
})
export class ListInjuryPhysoComponent implements OnInit {
  injuries: Injury[] = [];
  joueurs: any[] = [];
  id : number;
  searchTerm: string = '';
...
onFileSelected(event: Event): void {
  ...
  this.importInjuriesFromExcel(file);
}

importInjuriesFromExcel(file: File) {
  if (file) {
    this.serviceInjury.uploadCSV(file).subscribe(
      (response) => {
        console.log('Injuries uploaded successfully:', response);
        // Rechargez toutes les blessures après le téléchargement réussi, y compris les blessures archivées
        this.loadInjuries();
      },
      (error) => {
        console.error('Error uploading injuries from Excel:', error);
      }
    );
  } else {
